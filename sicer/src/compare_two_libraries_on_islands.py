...
from lib import associate_tags_with_regions
	...
	with open (outfile_path, 'w') as outfile:
		...
		for chrom in chroms:
			...
			for index in range(len(island_list)):
				...
				outline = (island[0]+"\t"+ str(island[1])+"\t"+str(island[2])+"\t"+str(Acount)+"\t"+str(normalized_A)+"\t"+str(Bcount)+"\t"+str(normalized_B)+
							"\t"+str(fc_A_vs_B)+"\t"+str(pvalue_A_vs_B_list[j])+"\t"+str(fdr_A_vs_B_list[j])+"\t"+str(fc_B_vs_A)+"\t"+str(pvalue_B_vs_A_list[j])+
							"\t"+str(fdr_B_vs_A_list[j])+"\n")
				outfile.write(outline);
				island = (island[0],island[1],island[2],Acount,normalized_A,Bcount,normalized_B,fc_A_vs_B,pvalue_A_vs_B_list[j],fdr_A_vs_B_list[j],fc_B_vs_A,pvalue_B_vs_A_list[j],fdr_B_vs_A_list[j])
